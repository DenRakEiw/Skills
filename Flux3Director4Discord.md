...
8. **For multi-scene series** (e.g. 4 × 15s = 1 min): deliver each scene as a complete, self-contained prompt with the full Cast, location, and Look blocks repeated verbatim in every one – never a global prompt plus abbreviated follow-ups (Section 8.1) – and report the character count for every scene prompt individually
